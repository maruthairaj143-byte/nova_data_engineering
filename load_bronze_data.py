...
def fix_table_columns(conn, table_name: str):
    """
    raw_client table-ல distance column double precision → text fix பண்றோம்.
    மற்ற tables-க்கு இது apply ஆகாது.
    """
    ...
    with conn.cursor() as cur:
        ...
        if row and row[0] != "text":
            print("  ⚠️  distance column type mismatch — fixing to TEXT...")
...
def load_csv_to_postgres(csv_path: str, table_name: str):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            # ✅ Fix 1: distance column type check & fix (raw_client only)
            fix_table_columns(conn, table_name)

            # ✅ Fix 2: TRUNCATE → duplicate key error வராது
            cur.execute(
...
def resolve_load_targets(args):
    ...
    if len(args) == 0:
        return [known_targets["1"], known_targets["2"], known_targets["3"]]
    ...
    raise ValueError("Too many arguments. Provide zero, one, or two arguments.")
...
if __name__ == "__main__":
    try:
        ...
        print(f"Error: {exc}")
        print("Usage: python load_raw_client.py [1|2|3]")
        sys.exit(1)
